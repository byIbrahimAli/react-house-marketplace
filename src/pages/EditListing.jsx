...
function EditListing() {
  ...
  // redirect if listing is not the user
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchListing = async () => {
      ...
      if (docSnap.exists()) {
        ...
        navigate('/')
        toast.error('Listing does not exist bud!')
      }
    }
  ...
  const onSubmit = async (e) => {
    ...
    if (geolocationEnabled) {
      ...
      if (location === undefined || location.includes('undefined')) {
        setLoading(false)
        toast.error(
          `Hmm, did you enter a proper address? I can't see it from the russian spy satellites.`
        )
      }
  ...
  const onMutate = (e) => {
    ...
    if (!e.target.files) {
      setFormData((prevState) => ({
        ...prevState,
        [e.target.id]: boolean ?? e.target.value, // if not boolean, use value
      }))
    }
  }
